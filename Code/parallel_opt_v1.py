...
@cuda.jit
def compute_alpha_kernel(alpha, beta, prev_alpha, d2alpha, unknown_seg):
    """
...
# Chưa xử lý được các pixel viền của block
@cuda.jit
def compute_alpha_kernel_shared(alpha, beta, prev_alpha, d2alpha, unknown_seg):
    """

    """
    s_alpha = cuda.shared.array((32,32), dtype=alpha.dtype)
    s_prev_alpha = cuda.shared.array((32,32), dtype=prev_alpha.dtype)
    s_d2alpha = cuda.shared.array((32,32), dtype=d2alpha.dtype)
...
def global_alpha_matting_shared(alpha, d2alpha, unknown_seg, width, height, iters = 50, threshold = 0.1, beta = 1):
    ...
    d_prev_alpha = cuda.to_device(prev_alpha)
    ...
    for _ in range(iters):
        ...
        compute_alpha_kernel_shared[grid_size, block_size](d_alpha, beta, d_prev_alpha, d_d2alpha, d_unknown_seg)
...
def matting_combined(tri, img_gray):
    ...
    mask_fg,fg,mask_bg,bg,mask_unknown,unknown = imgs
    alpha = global_alpha_matting_shared(estimate_alpha,d2alpha,mask_unknown, estimate_alpha.shape[1], estimate_alpha.shape[0])
...
# Copy mảng local ra 1 mảng riêng biệt chứ không dùng chỉ mục của mảng global
